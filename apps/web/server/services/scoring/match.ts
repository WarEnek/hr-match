...
function computeOverlapScore(requirement: string, evidence: CandidateEvidence) {
  ...
  const evidenceTokens = new Set([...normalizeTokens(evidence.text), ...evidence.tags.flatMap((tag) => normalizeTokens(tag))])
  let overlap = 0
...
export async function runMatchPipeline(event: H3Event, profileId: string, vacancyId: string) {
  ...
  const [{ data: vacancy, error: vacancyError }, { data: requirements, error: requirementsError }, { data: skills }, { data: experienceBullets }, { data: projectBullets }] = await Promise.all([
    supabase.from('vacancies').select('*').eq('id', vacancyId).eq('profile_id', profileId).maybeSingle(),
    supabase.from('vacancy_requirements').select('*').eq('vacancy_id', vacancyId).order('weight', { ascending: false }),
  ...
  if (requirementsError) {
    throw createAppError(500, 'Failed to load vacancy requirements.', { cause: requirementsError.message })
  }
  ...
  const mustHaveCoverage = mustHaveRequirements.length
    ? mustHaveRequirements.filter((item) => item.coverage_score >= 0.5).length / mustHaveRequirements.length
    : 0
  ...
  const domainFitHits = domainTerms.filter((term: string) => normalizeTokens(profileHeadline).includes(term.toLowerCase())).length
