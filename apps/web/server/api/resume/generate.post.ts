...
export default defineEventHandler(async (event) => {
  ...
  if (resumeError || !resume) {
    throw createAppError(500, 'Failed to create resume generation.', { cause: resumeError?.message })
  }
  ...
  if (evidenceLinks.length) {
    ...
    if (evidenceError) {
      throw createAppError(500, 'Failed to store evidence links.', { cause: evidenceError.message })
    }
  }
