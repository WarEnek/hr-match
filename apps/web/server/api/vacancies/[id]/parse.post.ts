...
export default defineEventHandler(async (event) => {
  ...
  if (vacancyUpdateError) {
    throw createAppError(500, 'Failed to update vacancy after parsing.', { cause: vacancyUpdateError.message })
  }
  ...
  if (requirementRows.length) {
    ...
    if (requirementError) {
      throw createAppError(500, 'Failed to store vacancy requirements.', { cause: requirementError.message })
    }
  }
