...
async function runMatch() {
  ...
  try {
    const response = await $fetch<{ analysis: MatchAnalysis }>(`/api/match/${vacancyId}`, { method: 'POST' })
    ...
    <section class="two-column">
      ...
      <div class="panel">
        ...
        <template v-if="parsed">
          <div class="pill-row">
            <span class="pill" v-for="item in parsed.must_have" :key="`must-${item}`">{{ item }}</span>
            <span class="pill" v-for="item in parsed.nice_to_have" :key="`nice-${item}`">{{ item }}</span>
          </div>
          <ul>
